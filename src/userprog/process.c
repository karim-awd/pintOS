...
static bool setup_stack(void **esp, char *pString[50], int i);
...
setup_stack(void **esp, char **argv, int argc) {
    ...
    if (kpage != NULL) {
        ...
        if (success) {
            *esp = PHYS_BASE;
            //setup the stack
            uint32_t **refArray[argc];

            for (int j = argc - 1; j >= 0; --j) {
                *esp = *esp - ((strlen(argv[j]) + 1) * sizeof(char));
                refArray[j] = malloc(sizeof(*esp));
                *refArray[j] = (uint32_t *) *esp;

                memcpy(*esp, argv[j], (strlen(argv[j]) + 1) * sizeof(char));
            }
            ...
            *esp = *esp - 4;
            ...
            for (int k = argc-1; k >= 0; --k) {
                *esp = *esp - 4* sizeof(char);
                memcpy(*esp, refArray[k], (sizeof(char)*4) );
